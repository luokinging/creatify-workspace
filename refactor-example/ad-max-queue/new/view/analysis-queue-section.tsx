...
export function AnalysisQueueSection() {
  ...
  const {
    ...
    isDataPending,
    isFetchingNextPage,
  } = useCombinedStore(vc.combinedStore, () => vc.analysisManager.pipelineQueryManager.state);
    ...
    <div className="flex-col gap-4">
      {items.map((item: AnalysisPipeline) => (
        <AnalysisQueueItemCard
          key={item.id}
          item={item}
          onApprove={vc.analysisManager.approveItem.bind(vc.analysisManager)}
          onReject={vc.analysisManager.rejectItem.bind(vc.analysisManager)}
          // todo: hide it temporarily until backend API is implemented
          // onRequestChanges={vc.analysisManager.requestChanges.bind(vc.analysisManager)}
        />
      ))}
      {isFetchingNextPage && (
