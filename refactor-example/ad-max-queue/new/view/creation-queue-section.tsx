...
function CreationQueueHeader() {
  ...
  const { testingBudget, isLowBudget, queueAccountState } = useCombinedStore(vc.combinedStore, () => ({
    testingBudget: vc.creationManager.testingBudget,
    ...
    <div className="flex items-center justify-between">
      <div className="flex items-center gap-3">
        <Button
          ...
          icon="add"
          onClick={async () => {
            await vc.creationManager.openRequestCreativesDialog(AdsPlatformEnum.Meta, vc.selectedAdAccountId);
          }}
        >
          Request Creatives
        </Button>
...
export function CreationQueueSection() {
  ...
  const {
    ...
    isDataPending,
    isFetchingNextPage,
  } = useCombinedStore(vc.combinedStore, () => vc.creationManager.pipelineQueryManager.state);

  const isGeneratingConcept = useCombinedStore(vc.combinedStore, () => vc.creationManager.isGeneratingConcept);
    ...
    <div className="flex-col gap-4">
      ...
      {items.map((item: CreationPipeline) => (
        <CreationQueueItemCard
          ...
          onReject={vc.creationManager.rejectItem.bind(vc.creationManager)}
          onGenerate={(id, concept, product) =>
            vc.creationManager.openRequestCreativesDialog(
              AdsPlatformEnum.Meta,
              vc.selectedAdAccountId,
              id,
              concept,
              product
            )
