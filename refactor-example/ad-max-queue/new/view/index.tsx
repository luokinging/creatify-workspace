...
import { IPlatformIntegrationService } from '@/feature/services/experimental/experimental-ads-platform-service/platform-integration-service.type';
...
import { QueuePageViewController } from '../../manager/queue/queue-page-view-controller';
...
function QueuePageContent() {
  ...
  const { activeTab, totalPending, creationCount, analysisCount, isLowBudget, budgetPercentage, selectedAdAccountId } =
    useCombinedStore(vc.combinedStore, () => ({
      ...
      totalPending:
        vc.creationManager.pipelineQueryManager.items.length + vc.analysisManager.pipelineQueryManager.items.length,
      creationCount: vc.creationManager.pipelineQueryManager.items.length,
      analysisCount: vc.analysisManager.pipelineQueryManager.items.length,
      isLowBudget: vc.creationManager.isLowBudget,
      budgetPercentage: vc.creationManager.budgetPercentage,
        ...
        <div className="flex items-start justify-between gap-4">
          ...
          <div className="flex items-center gap-2">
            {queueAccountState.status === 'account-ready' ? (
              <>
                {selectAdsAccount?.source.status === 'syncing' && (
...
export function QueuePage() {
  const platformIntegrationService = useServices(IPlatformIntegrationService);
  const transientDataService = useServices(ITransientDataService);
  const brandSpaceService = useServices(IBrandSpaceService);
  const [vc] = useState(
    () => new QueuePageViewController(platformIntegrationService, transientDataService, brandSpaceService)
  );
