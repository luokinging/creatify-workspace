...
export function KnowledgeBaseSection() {
  ...
  const { isLoading, error, data: rules } = useCombinedStore(vc.combinedStore, () => vc.knowledgeRulesState);
  ...
  const handleRefresh = async () => {
    ...
    try {
      ...
      if (selectedAdAccountId) {
        fetchPromises.push(vc.knowledgeRulesQueryClient.fetch());
      }
  ...
  const lastUpdated = insightsLastUpdated || vc.lastRuleUpdated;
      ...
      <Card className="flex-col gap-4 p-5">
        ...
        <div className="flex items-center justify-between border-line-2 border-b pb-3">
          ...
          <div className="flex items-center gap-3">
            ...
            <Button size="sm" variant="ghost" icon="comment" onClick={() => vc.openInstructDialog(snapshotId)}>
              Instruct
            </Button>
          </div>
        </div>
