...
2. These indexes MAY evolve, but the `index.md` entry point is guaranteed. Every feature spec MUST state which indexes (and downstream docs) were consulted, differentiating frontend vs. backend scope so guidance never gets conflated.
3. Optional rule documents under other directories (.local, etc.) MUST have their front-matter `description` reviewed before deciding relevance, exactly as the frontend index prescribes.
