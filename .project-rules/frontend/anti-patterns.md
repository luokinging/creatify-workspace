...
> **🔴 硬性规定**：如果 `useMemo`、`useCallback` 或 `useEffect` 的代码块超过 **10 行**，必须将其提取到 **ViewController** 或 **Manager** 中。组件内不应包含复杂的内联逻辑。
...
- 逻辑分散，难以追踪。
  **正确做法**：
- 将业务逻辑移至 **Manager**。
- `useEffect` 仅用于同步外部系统（如订阅事件、手动 DOM 操作）。
...
- **保持组件纯净**：组件只负责 UI。
- **逻辑下沉**：复杂逻辑去 Manager。
- **适度优化**：不要过早优化（Premature Optimization）。
